-- Migration: Fix transaction total validation logic
-- The validation was comparing sum of line totals (after line discounts) with total_before_discount
-- It should compare with total_after_discount instead
...
COMMENT ON FUNCTION public.create_transaction_with_items(JSONB, JSONB[]) IS 'Atomically creates a transaction with multiple items, validates totals correctly, and updates inventory only for in-store sales';
